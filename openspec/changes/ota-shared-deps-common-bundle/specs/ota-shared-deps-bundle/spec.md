...
The build pipeline SHALL produce a dedicated **shared** OTA split bundle `ota_shared.<version>.ios.jsbundle` containing cross-feature React Native dependencies (including `@react-navigation/*`, `react-native-screens`, `react-native-gesture-handler`, and their transitive split-graph dependencies required by Remote navigators).
