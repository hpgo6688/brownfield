...
The server SHALL accept upload of the shared bundle artifact (e.g. `featureId: "shared"` or dedicated upload route) and persist it for manifest distribution alongside per-feature bundles.
...
- **THEN** manifest `sharedBundle` (or equivalent) exposes url, version, hash, and sizeBytes
